...
import { FileText, Folder, Shield, Smartphone } from 'lucide-react'
    ...
    return (
        <div className=' w-full h-screen flex items-center justify-center  '>
            <div className=' mx-auto  flex  rounded-md  '>
                <div className=' px-10 py-11  bg-[#155DFC] rounded-l-md '>
                    <div className=' mb-8  '>
                        <Link to={'/'}>
                ...
                <div className=' border-t w-lg  bg-white shadow-2xl  p-11  rounded-r-md '>
                    <div>
                        <div className=' flex bg-[#f3f4f6] p-1  rounded-lg '>
                            <button onClick={() => setCurrentAuthPage("signin")} className={`text-sm flex-1  cursor-pointer outline-0   transition-all font-inter font-medium px-4  py-2.5 hover:text-black    rounded-md ${currentAuthPage === "signin" ? 'bg-white text-black ' : "bg-transparent text-[#4F555C]"}`}>Sign In</button>
                            <button onClick={() => setCurrentAuthPage("create")} className={` text-sm flex-1 cursor-pointer outline-0  transition-all font-inter font-medium px-4 py-2.5 hover:text-black  rounded-md   text-[#4F555C] ${currentAuthPage === "create" ? 'bg-white text-black ' : "bg-transparent text-[#4F555C]"}`}>Create Account</button>
                        </div>

                        <div className=' mt-7'>
                            {currentAuthPage === "signin" ? <Signin/> : <Signup/>}
                        </div>
                    </div>
                </div>
            </div>
